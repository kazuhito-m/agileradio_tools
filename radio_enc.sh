...
eyeD3 --to-v2.4 --set-encoding=utf8 --artist="${ARTIST}" --album="${TITLE_BASE}" --title="${FULL_TYTLE}" --year=${MAKE_YEAR} --publisher=agileradio --comment=jp:abc:"${TITLE_COMMENT}" --add-image=./agileradio.jpg:OTHER:agileradio.jpg  --set-user-url-frame="Description:${SITE_URL}" --set-user-text-frame="Description:${ARTIST}" --set-text-frame=TPE2:"${A_ARTIST}" --set-text-frame=TCOM:"${COMMPOSER}" --set-text-frame=TDAT:"${MAKE_YEAR}" --set-text-frame=TCOP:"${A_ARTIST}" --set-text-frame=TENC:lame ${RESULT_FILE}
